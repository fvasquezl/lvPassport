...
it('guest users can fetch a single category', function () {
    ...
    $this->jsonApi()
        ->get(route('api.v2.categories.show', $category))
        ->assertOk();
});
...
it('authenticated users can fetch a single category', function () {
    ...
    $this->jsonApi()->get(route('api.v2.categories.show', $category))
        ->assertOk()
        ->assertJson([
            'data' => [
                'type' => 'categories',
                ...
                'links' => [
                    'self' => route('api.v2.categories.show', $category),
                ],
            ],
        ]);
});
...
it('can fetch all categories', function () {
    ...
    $this->jsonApi()->get(route('api.v2.categories.index'))
        ->assertOk()
